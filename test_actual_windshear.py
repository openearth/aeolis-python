"""
Test the actual WindShear.interpolate() method to verify the optimization.
This creates a backup of the old method and compares outputs.
"""
...
def interpolate_old_padded(self, x, y, z, xi, yi, z0):
    """OLD interpolate method WITH padding - saved for comparison"""
    ...
    x, y = WindShear.rotate(x, y, angle, origin=(self.x0, self.y0))
    xi, yi = WindShear.rotate(xi, yi, angle, origin=(self.x0, self.y0))
    
    # Rotate 180 deg if necessary
    if not np.all(sorted(y[:,0]) == y[:,0]) and not np.all(sorted(x[0,:]) == x[0,:]):
        x, y = WindShear.rotate(x, y, 180, origin=(self.x0, self.y0))
        xi, yi = WindShear.rotate(xi, yi, 180, origin=(self.x0, self.y0))

    # Concatenate
    xyi = np.concatenate((yi.reshape((-1,1)),
                          xi.reshape((-1,1))), axis=1)

    # OLD: Interpolate WITH padding
    if self.istransect:
        zi = np.interp(xi.flatten(), x.flatten(), z.flatten()).reshape(xi.shape)
    else:
        # OLD METHOD: Use padding
        pad_w = max(x.shape[0], x.shape[1])
        ...
        inter = scipy.interpolate.RegularGridInterpolator(
            ...
            z_pad, bounds_error=False, fill_value=z0
        )
...
try:
    ...
    # Set computational grid for wind direction
    ...
    # Test interpolation with both methods
    print("\n" + "="*80)
    print("TESTING INTERPOLATION METHODS")
    print("="*80)
    
    print("\nInterpolating with OLD method (with padding)...")
    zi_old = interpolate_old_padded(ws, x, y, z, xi, yi, 0.0)
    
    print("Interpolating with NEW method (optimized, no padding)...")
    zi_new = ws.interpolate(x, y, z, xi, yi, 0.0)
    ...
    print("\nDifference statistics by region:")
    print(f"  Interior (no extrapolation): max={abs_diff[10:-10,10:-10].max():.6e}, mean={abs_diff[10:-10,10:-10].mean():.6e}")
    print(f"  Edges (with extrapolation): max={abs_diff.max():.6e}")
...
"""
    ...
    plt.suptitle(f'WindShear.interpolate() Comparison\n{os.path.basename(config_file)}', fontsize=14, fontweight='bold')
    
    output_file = 'windshear_interpolation_comparison.png'
    ...
    if abs_diff.max() < 1e-8:
        ...
        print("  → Differences are negligible for physical applications")
        print("  → Optimization is safe to deploy")
    elif rel_diff.max() < 1.0:
        print("~ SIMILAR: Methods produce similar results with small differences")
        print("  → Review differences to ensure they are acceptable")
    else:
