...
#    nf = p['nfractions']
    ...
    mg = (s['moist'][:,:] * p['rhow'] / (p['rhop'] * (1. - p['porosity'])))#.repeat(nf, axis=2)
    ix = mg > 0.005
    ...
    if p['method_moist_threshold'].lower() == 'belly_johnson':
        ...
        logger.log_and_raise('Unknown moisture formulation [%s]' % p['method_moist_threshold'], exc=ValueError)
...
## CH: I don't think this implementation of Arens humidity threshold is correct - it should only be applied
## for dry days... I think the humidity function will be redudant with new surface moisture module
