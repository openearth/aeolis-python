...
app= typer.Typer(add_completion=False, pretty_exceptions_enable=True, help="AeoLiS, a process-based model for simulating supply-limited aeolian sediment transport")

@app.command(help='runs model for simulating supply-limited aeolian sediment transport.')
def run(
...
@app.command(name="examples",
             help="sets up some examples for AeoLiS.")
def examples(
    dir: Annotated[str, typer.Argument(
        help="directory for the examples' data. It will be created if it does not exist."
    )]
):
