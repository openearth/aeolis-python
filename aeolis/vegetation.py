...
def vegshear_okin(s, p):
    ...
    wind_reduce = np.zeros([nx, nx])
    for i in range(nx):
        dist = x [0,:]- x[0,i]
        dist = np.float64(dist)
        #ht = hta[i,:]
        height = ht[0,i]
        #print('number of heights', np.shape(x))

        if height > 0:
            for j in range(nx):
                val1 = 1 - B
                val2 = 1 - np.exp(-C * dist[j])
                val2 = 0

                val3 = np.float64(val1) * np.float64(val2)
                val4 = val3 + np.float64(B)
                val5 = 1 - val4
                val6 = val5 * np.float64(height)
                okin = val6
                #print('val1', val1)
                #print('dist', dist[j])
                ...
                #okin = [1 - np.float64(val1) * np.float64(val2) + np.float64(B)] * np.float64(height)
                udir = s['udir'][0, i]
                #print('udir', udir)

                if udir < 90 and udir > -90:
                    ix = dist < 0
                    okin = 0
                else:
                    ix = dist > 0
                ...
                wind_reduce[i, j] = okin

    wind_reduce_all = np.nansum(wind_reduce, axis=1)

    #print('number of winds', np.shape(wind_reduce_all))

    #wind_reduce_all >= 1 = 1
    ix = wind_reduce_all > 1
    wind_reduce_all[ix] = 1

    wind_factor = 1 - wind_reduce_all
    wind_factor2 = np.zeros(s['ustar'].shape)
    wind_factor2[0, :] = wind_factor

    #print('number of wind factors', np.shape(wind_factor2))
    #print('number of ustar', np.shape(s['ustar']))
...
def initialize_roots(s, p):
    ...
    #initialize veg model
    xgrid = s['x']
    zgrid = lowerlim:0.1:maxlim
    s['nzveg'] = np.size(zgrid)
    s['XGRID'], s['ZGRID'] = np.meshgrid(xgrid, zgrid)
    s['VEG_BURIED_AGE'] = np.zeros(np.shape(XGRID))
    s['PERC_BURIED_AGE'] = np.zeros(np.shape(XGRID))
    s['ROOTS'] = np.zeros(np.shape(XGRID))
    s['root_depth'] = s['z'] - initial_rooting_depth
    s['surface_age'] = np.zeros(np.shape(s['z']))
    s['root_age'] = np.zeros(np.shape(s['z']))

    #NaN out areas above air or based on other criteria
    root_depth < min_rooting_depth = np.nan()

    for i in range(nx):
        for j in range(s['nzveg']):
            if 
    ...
    # Reduction of vegetation growth due to sediment burial
    s['dhveg'][ix] = p['V_ver'] * (1 - s['hveg'][ix] / p['hveg_max']) - np.abs(s['dzbveg'][ix] - p['dzb_opt']) * p[
        'veg_gamma']  # m/year
